/* ============================================================
   SURVEY PORTAL RADAR — Gráfico radar SVG de la vista 1.0.
   Expone window.SurveyPortalRadar.
   NOTA: se mantiene el renderizado interno (radarRender) en lugar
   de SurveyRadarChart externo porque NO es compatible con la vista
   del portal: (1) el select de categorías del portal es
   #srv-cat-radar (ya poblado y enlazado por portal-filters) y el
   externo busca #filter-categoria-radar; (2) el tooltip del portal
   usa #radarTooltip.bar-tooltip (estilos propios) mientras el
   externo usa SurveyTooltip (#tooltip.tooltip, sin estilos en el
   portal); (3) el insight del portal renderiza formatDimensionName
   con <span><i>Software</i></span> sin escapar, el externo lo
   escapa. Mismas funciones internas adaptadas (sin copias _new).
   ============================================================ */
(function () {
  'use strict';
  ...
  function formatDimensionNameForAttr(d) {
    return d.toLowerCase().replace(/[^a-z0-9]+/g, '-');
  }
  function formatDimensionNameSVG(dim, maxlen) {
  ...
  function dimensionAplica(rows, dim) {
    return rows.some((r) => r.dimension === dim && sumKeys(r, RADAR_SAT_KEYS) > 0);
  }

  // Tooltip local del radar (div flotante; HTML rico: pie + barras)
  function showRadarTooltip(e, html) {
    let tip = document.getElementById('radarTooltip');
    if (!tip) {
      tip = document.createElement('div');
      tip.id = 'radarTooltip';
      tip.className = 'bar-tooltip radar-tooltip';
  ...
  function updateInsightFortaleza(allDims, fac, car, cic) {
    ...
    const fortalezas = allDims.filter((d) => d.pct >= RADAR_META_CSAT).sort((a, b) => b.pct - a.pct);
    const adecuados = allDims.filter((d) => d.pct >= RADAR_META_PONDERADO && d.pct < RADAR_META_CSAT).sort((a, b) => b.pct - a.pct);
    const atencion = allDims.filter((d) => d.pct < RADAR_META_PONDERADO).sort((a, b) => a.pct - b.pct);
    ...
    const cleanContexto = esc(contexto);
    const fmtP = (v) => fmtNum(v, 2) + ' %';
    ...
    if (hayFiltro) {
      ...
      if (fortalezas.length) {
        ...
        txt += fortalezas
          ...
          .join(', ');
        txt += `. En total, <strong>${fortalezas.length}</strong> de ${allDims.length} dimensiones superan el ${RADAR_META_CSAT} %.`;
      } else {
        txt += `Ninguna dimensión alcanza el umbral de <strong>${RADAR_META_CSAT} %</strong> (Fortaleza). `;
        if (adecuados.length) {
      ...
      if (atencion.length) {
        ...
        txt += atencion
          ...
          .map((d) => `<strong>${fmtD(d.dim)}</strong> (${fmtP(d.pct)})`)
          .join(' y ');
        txt += ` por estar debajo del ${RADAR_META_PONDERADO} %.`;
      }
    } else {
      if (fortalezas.length) {
        ...
        txt += `En total, <strong>${fortalezas.length}</strong> de ${allDims.length} dimensiones se encuentran en rango de Fortaleza (≥${RADAR_META_CSAT} %).`;
      } else {
        txt += `Actualmente ninguna dimensión alcanza el umbral de <strong>${RADAR_META_CSAT} %</strong> (Fortaleza).`;
      }
      if (atencion.length) {
  ...
  // Renderiza el Radar Chart en el SVG (adaptación autocontenida).
  function radarRender(options) {
    const o = options || {};
    const svgId = o.svgId || 'radar-chart';
    const filteredDimensions = o.filteredDimensions || [];
    const rawDimensions = o.rawDimensions || [];
    const fac = o.fac || '';
    const car = o.car || '';
    const cic = o.cic || '';
    const svg = $(svgId);
    ...
    // Filtro de categorías: leer del select srv-cat-radar (inicializado con SurveyMultiselect)
    const selCat = $('srv-cat-radar');
    ...
    filteredDimensions.forEach((r) => {
      ...
      d.total += sumKeys(r, RADAR_SAT_KEYS);
      d.top3 += sumKeys(r, RADAR_SAT_TOP3_KEYS);
    ...
    // Separar por lados (derecho e izquierdo) para resolver colisiones verticales
    ...
    labels.forEach((l) => {
      ...
      parts.push(`<text x="${l.x}" y="${l.y}" font-size="10" font-weight="500" fill="#6B7280" style="cursor:pointer;"
                  ...
                  data-pct="${fmtDecimal(l.pct, 2)}"
                  data-t2b="${fmtDecimal(l.top2box, 2)}"
                  data-pond="${fmtDecimal(l.ponderado, 2)}">${formatDimensionNameSVG(l.dim, RADAR_LABEL_MAXLEN)}</text>`);
    });
    ...
    allDims.forEach((d, i) => {
      ...
      parts.push(`<circle cx="${ox}" cy="${oy}" r="4" fill="${color}" style="cursor:pointer;opacity:0"
                  ...
                  data-pond="${fmtDecimal(d.ponderado, 2)}">
    ...
    svg.querySelectorAll('text[data-dim], circle[data-dim], line[data-dim]').forEach((el) => {
      ...
      el.addEventListener('mouseleave', () => hideRadarTooltip());
      el.addEventListener('focus', (e) => {
        ...
        if (d) {
          ...
          const pct = el.getAttribute('data-pct');
          showRadarTooltip(fakeEvent, esc(dim) + ': ' + pct + '% de satisfacción');
        }
      });
      el.addEventListener('blur', () => hideRadarTooltip());
    });
  ...
  // Render del radar independiente del resto de la vista 1.0.
  function renderRadarIndependiente() {
    const g = window.__surveyFilter ? window.__surveyFilter.radar : null;
    const fac = g ? g.fac : '';
    const car = g ? g.car : '';
    const rawCic = g ? g.ciclos : '';
    const ciclos = (Array.isArray(rawCic) && rawCic.length) ? rawCic : '';
    const D = _data.getSurveyData();
    const filtered = _data.filtrarDatos(D.dimensiones_raw, fac, car, ciclos);
    radarRender({
      svgId: 'radar-chart',
      filteredDimensions: filtered,
      rawDimensions: D.dimensiones_raw,
      fac: fac,
      car: car,
      cic: ciclos
    });
  ...
  window.SurveyPortalRadar = {
    renderRadarIndependiente: renderRadarIndependiente,
